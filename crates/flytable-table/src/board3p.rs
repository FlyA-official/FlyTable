//! 3-player game core.
//!
//! 3 seats, a 108-tile wall (manzu only 1m/9m), nukidora and no chi. Holds perfect
//! information and projects an imperfect-information view for any seat.
//!
//! Shared with 4-player: wall geometry [`crate::wall`], seat state [`crate::player`],
//! seat views [`crate::view`], the settlement bridge [`crate::scoring`] and the rules
//! core `flytable-core`. The progression itself is separate: 3-seat rotation,
//! nukidora and the absence of chi are all expressed here.
...
/// How a 3-player hand ended.
#[derive(Debug, Clone)]
pub enum KyokuOutcome3p {
    Hora {
        winner: u8,
        from: Option<u8>,
        score: flytable_core::score::FullScore,
    },
    /// Double ron on one discard. `first` guarantees the result is not empty.
    MultiHora {
    ...
    AbortiveRyukyoku {
        reason: AbortiveRyukyokuReason3p,
    },
}

/// Reason for a 3-player abortive draw.
///
/// 3-player supports kyuushu kyuuhai and four kans. These 4-player draws cannot occur:
/// - Four identical winds (SuufonRenda): needs four players.
/// - Triple ron (Sanchahou): at most two players can ron one discard.
/// - Four riichi (Suucha Riichi): needs four players.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum AbortiveRyukyokuReason3p {
    KyuushuKyuuhai,
    Suukaikan,
}
/// 3-player table.
pub struct Board3p {
    ...
    /// Nukidora count per seat.
    pub nuki: [u8; SEATS],
    pub log: Vec<flytable_event::Event3p>,
    ...
    /// Riichi declared with its discard, awaiting the response window.
    pending_riichi: Option<u8>,
    /// Kan or nukidora declaration written to the log, awaiting ron responses, before the replacement draw.
    pending_robbery: Option<PendingRobbery>,
    /// Dora from open or added kans still to be revealed.
    pending_kan_dora: u8,
    /// Liable player for daisangen / daisuushi, indexed by winner seat.
    pao: [Option<u8>; SEATS],
...
impl Board3p {
    ...
    pub fn start(seed: (u64, u64), scores: [i32; SEATS]) -> Self {
        let wall = Wall::shuffled(crate::tileset::tileset_3p(), SEATS, seed);
    ...
    pub fn start_with_rule_profile(
        ...
        let tiles = crate::tileset::tileset_3p_with_red_fives(rule_profile.red_fives(SEATS))?;
    ...
    pub fn with_rule_profile(mut self, rule_profile: RiichiRuleProfile) -> Result<Self, String> {
        ...
        if actual != expected {
            return Err(format!(
                "3p wall red fives do not match the rule profile: wall={actual:?}, profile={expected:?}"
            ));
        }
    ...
    ///
    /// `bakaze`: 0 = East, 1 = South, 2 = West, 3 = North. The core stays at three seats;
    /// the mjai fourth-seat placeholder only exists in the boundary adapter.
    pub fn from_wall_with_state(
        ...
        let players = (0..SEATS)
            .map(|s| PlayerState::new_sanma(s as u8, wall.haipai(s)))
            .collect();
        ...
        board.log.push(flytable_event::Event3p::StartKyoku {
            bakaze,
    ...
    pub fn draw_for_turn(&mut self) -> Option<Tile> {
        ...
        self.log.push(flytable_event::Event3p::Tsumo {
            actor: self.turn,
            pai: t,
        });
        Some(t)
    ...
    /// Applies the current seat's turn action. `Some` means the hand ended.
    pub fn apply_turn(&mut self, action: TurnAction) -> Result<Option<KyokuOutcome3p>, String> {
        if self.terminal {
            return Err("the hand has ended".into());
        }
        if self.pending_robbery.is_some() {
            return Err("a kan or nukidora declaration is still awaiting ron responses".into());
        }
        if self.turn as usize >= SEATS {
        ...
        match &action {
            ...
            TurnAction::Nukidora
                if self.wall.live_remaining() == 0 || self.wall.supplement_remaining() == 0 =>
            {
                return Err("no supplementary tiles left".into());
            }
            _ => {}
        }
        ...
        match action {
            ...
            TurnAction::Riichi { tile, tsumogiri } => {
                ...
                self.log
                    .push(flytable_event::Event3p::Reach { actor: self.turn });
                self.do_discard(self.turn, tile, tsumogiri)?;
            ...
            TurnAction::Tsumo => {
                ...
                self.terminal = true;
                Ok(Some(KyokuOutcome3p::Hora {
                    winner: seat,
                    from: None,
                    score,
                }))
            }
            TurnAction::Nukidora => self.do_nukidora(self.turn),
            TurnAction::Ankan { tile } => self.do_ankan(self.turn, tile),
            TurnAction::Kakan { tile } => self.do_kakan(self.turn, tile),
            TurnAction::KyuushuKyuuhai => self.do_kyuushu_kyuuhai(),
        }
    }

    fn do_kyuushu_kyuuhai(&mut self) -> Result<Option<KyokuOutcome3p>, String> {
        if !self.can_kyuushu_kyuuhai(self.turn) {
            return Err("kyuushu kyuuhai condition not met".into());
        }
        self.log.push(flytable_event::Event3p::Ryukyoku {
            deltas: Some([0; SEATS]),
        });
        self.terminal = true;
        Ok(Some(KyokuOutcome3p::AbortiveRyukyoku {
            reason: AbortiveRyukyokuReason3p::KyuushuKyuuhai,
        }))
    }
    ...
    fn do_discard(&mut self, seat: u8, tile: Tile, tsumogiri: bool) -> Result<(), String> {
        ...
        self.log.push(flytable_event::Event3p::Dahai {
            actor: seat,
            pai: tile,
            tsumogiri,
        });
    ...
    fn reveal_pending_kan_dora(&mut self) {
        while self.pending_kan_dora > 0 {
            ...
            self.log.push(flytable_event::Event3p::Dora {
                dora_marker: marker,
            });
        }
    }
    ...
    /// revealed at this kan or nukidora declaration, so a robbing player gets it. Tenhou
    /// (flag false) keeps revealing after the window and before the replacement draw.
    ...
    fn accept_pending_riichi(&mut self) {
        ...
        *score -= 1_000;
        ...
        self.log
            .push(flytable_event::Event3p::ReachAccepted { actor });
    }
    ...
    pub fn clear_last_discard(&mut self) -> Option<KyokuOutcome3p> {
        if self.terminal {
            return None;
        }
    ...
    /// Verify that the response window is backed by the same, still uncalled
    /// tile at the end of the discarder river.  This is checked before any
    /// mutation because `last_discard` is publicly inspectable/injectable.
    ...
    /// Nukidora: move a North from hand to the meld area and draw a supplementary tile (no new indicator).
    fn do_nukidora(&mut self, seat: u8) -> Result<Option<KyokuOutcome3p>, String> {
        if self.wall.live_remaining() == 0 || self.wall.supplement_remaining() == 0 {
            return Err("no supplementary tiles left".into());
        }
        let north: Tile = "N".parse().unwrap();
        let p = &mut self.players[seat as usize];
        let pos = p
            .hand
            .iter()
            .position(|t| t.kind() == north.kind())
            .ok_or("no North in hand")?;
        let p = &mut self.players[seat as usize];
        let tile = p.hand.remove(pos);
        p.drawn_tile = None;
        p.melds.push(flytable_core::meld::Meld::Nukidora { tile });
        self.nuki[seat as usize] += 1;
        self.flush_pending_kan_dora_for_declaration();
        self.log.push(flytable_event::Event3p::Nukidora {
            actor: seat,
            pai: tile,
        });
        // The previous delayed kan dora waits until the nukidora robbing window passes (see resolve_pending_robbery_passes).
    ...
    fn do_ankan(&mut self, seat: u8, tile: Tile) -> Result<Option<KyokuOutcome3p>, String> {
        ...
        let p = &mut self.players[seat as usize];
        let k = tile.kind();
        if p.hand.iter().filter(|t| t.kind() == k).count() < 4 {
        ...
        self.flush_pending_kan_dora_for_declaration();
        self.log.push(flytable_event::Event3p::Ankan {
            actor: seat,
            consumed,
        });
    ...
    fn do_kakan(&mut self, seat: u8, tile: Tile) -> Result<Option<KyokuOutcome3p>, String> {
        if self.wall.rinshan_remaining() == 0 {
            return Err("no replacement tiles left".into());
        }
        let k = tile.kind();
        ...
        self.flush_pending_kan_dora_for_declaration();
        self.log.push(flytable_event::Event3p::Kakan {
            actor: seat,
            pai: added,
    ...
    /// Current public kan or nukidora declaration; while `Some`, no normal draw or discard.
    pub const fn pending_robbery(&self) -> Option<PendingRobbery> {
        self.pending_robbery
    }

    /// Nobody robbed the declaration: only now is ippatsu broken and the replacement drawn.
    pub fn resolve_pending_robbery_passes(&mut self) -> Result<Option<KyokuOutcome3p>, String> {
        if self.terminal {
            return Err("the hand has ended".into());
        }
        let pending = self
            .pending_robbery
            .take()
            .ok_or("no pending kan or nukidora declaration")?;
        self.clear_all_ippatsu();
        // Four kans on a nukidora depends on the platform (`suukaikan_on_kita_declaration`).
        // Tenhou ends the hand as soon as the robbing window passes
        // (`ankan -> dora -> tsumo -> kita -> ryukyoku`, without a replacement draw). Mahjong
        // Soul does not: the replacement is drawn and a following rinshan tsumo still
        // scores. The discard path (`clear_last_discard`) is the same for both.
        //
        // With Mahjong Soul's kan dora timing (reveal at declaration), the fourth kan takes
        // effect at the declaration, which is consistent.
        if pending.kind == RobberyKind::Nukidora
            && self.rule_profile.suukaikan_on_kita_declaration()
            && self.suukaikan_condition_met()
        {
            return Ok(Some(self.abort_suukaikan()));
        }
        ...
        self.last_draw_had_ippatsu = false;
        let r = match pending.kind {
            RobberyKind::Nukidora => self
                .wall
                .draw_supplement()
                .ok_or("no supplementary tiles left")?,
            RobberyKind::Ankan | RobberyKind::Kakan => self
                .wall
                .draw_rinshan()
        ...
        match pending.kind {
            RobberyKind::Ankan => {
                if let Some(dm) = self.wall.reveal_kan_dora() {
                    self.log
                        .push(flytable_event::Event3p::Dora { dora_marker: dm });
                }
            }
    ...
    /// excluded). Once true it stays true, and the fourth kan player's discard or
    /// nukidora ends the hand when its window clears, so "condition met and a pending
    /// discard" identifies the ron-only window without extra state.
    fn suukaikan_condition_met(&self) -> bool {
        // Shared with 4P in `crate::timing`.
    ...
    fn abort_suukaikan(&mut self) -> KyokuOutcome3p {
        self.log.push(flytable_event::Event3p::Ryukyoku {
            deltas: Some([0; SEATS]),
        });
        self.terminal = true;
        KyokuOutcome3p::AbortiveRyukyoku {
            reason: AbortiveRyukyokuReason3p::Suukaikan,
        }
    }
    ...
    /// Legal responses of a seat to `last_discard` (3-player: no chi; pon, open kan, ron).
    pub fn legal_reactions(&self, seat: u8) -> Vec<crate::action::ReactionAction> {
        ...
        let k = tile.kind();
        // Ron: complete hand, no furiten and a yaku (same as settlement in apply_ron).
        ...
        if same.len() >= 2 {
            for consumed in pon_pair_combos(&same) {
    ...
    /// Legal responses to a kan or nukidora declaration: ron only.
    ...
    /// Atomically applies one or two rons on the same discard.
    pub fn apply_rons(
        &mut self,
        winners: &[u8],
        from: u8,
        ...
        if results.is_empty() {
            Ok(KyokuOutcome3p::Hora {
                winner: first.winner,
                from: first.from,
                score: first.score,
            })
        } else {
            Ok(KyokuOutcome3p::MultiHora {
                first,
                additional: results,
            })
        }
    ...
    /// Atomically applies one or two rons on the current kan or nukidora declaration.
    pub fn apply_robbery_rons(&mut self, winners: &[u8]) -> Result<KyokuOutcome3p, String> {
        if self.terminal {
            return Err("the hand has ended".into());
        }
        let pending = self
        ...
        for winner in ordered {
            ...
            if !self.can_robbery_ron(winner, pending) {
                return Err(format!("seat {winner} cannot ron the declared tile"));
            }
            ...
            let score = self
                ...
                .ok_or_else(|| {
                    format!("seat {winner} has no yaku; cannot ron the declared tile")
                })?;
            results.push(crate::HoraResult {
                winner,
    ...
    /// Scripted entry point: declare an added kan and have `winner` rob it immediately.
    ...
    pub fn apply_pon(&mut self, actor: u8, consumed: [Tile; 2]) -> Result<(), String> {
        ...
        self.clear_all_ippatsu();
        self.log.push(flytable_event::Event3p::Pon {
            actor,
            target: from,
            pai: tile,
            consumed,
    ...
    pub fn apply_daiminkan(&mut self, actor: u8) -> Result<Option<KyokuOutcome3p>, String> {
        if self.terminal {
            return Err("the hand has ended".into());
        }
        if actor as usize >= SEATS {
        ...
        self.players[from as usize].mark_latest_discard_called()?;
        self.update_pao_after_open_honor_call(actor, from, tile);
        self.log.push(flytable_event::Event3p::Daiminkan {
            actor,
            target: from,
            pai: tile,
            consumed,
        ...
        if let Some(r) = self.wall.draw_rinshan() {
            ...
            self.log
                .push(flytable_event::Event3p::Tsumo { actor, pai: r });
            Ok(None)
        } else {
    ...
    pub fn pao_payer(&self, winner: u8) -> Option<u8> {
        ...
        self.pao[winner as usize]
    }

    /// Pao liability: 3-player only has daisangen pao, always 1 part (same interface as 4-player).
    pub fn pao_liability(&self, winner: u8) -> Option<(u8, u8)> {
        self.pao_payer(winner).map(|seat| (seat, 1))
    }
    ...
    fn update_pao_after_open_honor_call(&mut self, actor: u8, from: u8, called: Tile) {
        ...
        if (31..=33).contains(&called_kind) && honor_triplets[4..].iter().all(|&v| v)
            || (27..=30).contains(&called_kind) && honor_triplets[..4].iter().all(|&v| v)
        {
            self.pao[actor as usize] = Some(from);
        }
    }
    ...
    fn can_robbery_ron(&self, seat: u8, pending: PendingRobbery) -> bool {
        ...
        match pending.kind {
            ...
            RobberyKind::Nukidora if !self.rule_profile.allows_nukidora_ron() => {
                return false;
            }
            RobberyKind::Ankan | RobberyKind::Nukidora => {}
        }
        let p = &self.players[seat as usize];
    ...
    fn settle_with_flags(
        ...
        let input = crate::scoring::ScoringInput {
            ...
            nuki_count: self.nuki[winner as usize],
            is_sanma: true,
        };
        crate::scoring::settle(&input)
    }

    /// Seat wind (dealer is East; 3-player seats are East, South and West, North is a guest wind).
    /// Public for the same reason as in 4-player: the match log writes Tenhou yaku IDs by wind.
    ...
    fn push_hora_event(&mut self, winner: u8, from: u8, score: &flytable_core::score::FullScore) {
        ...
        let scoring = HoraScoring {
            ...
            nuki_dora_han: score.nuki_dora_han,
            yaku_flags: crate::scoring::yaku_flags_from_score(score),
        };
        self.log.push(flytable_event::Event3p::Hora {
            actor: winner,
            target: from,
            deltas: None,
            ura_markers,
    ...
    pub fn ryukyoku(&mut self) -> KyokuOutcome3p {
        let tenpai = self
            .players
            .iter()
...
fn pon_pair_combos(same_kind_tiles: &[Tile]) -> Vec<[Tile; 2]> {
    let mut out = Vec::new();
...
impl crate::product::GameServer for Board3p {
